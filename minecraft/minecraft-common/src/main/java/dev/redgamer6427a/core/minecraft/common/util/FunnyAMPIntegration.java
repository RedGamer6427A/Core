...
public abstract class FunnyAMPIntegration extends AMPIntegration {


    protected FunnyAMPIntegration(String key) {
        super(key);
    }
    ...
    protected AMPStatus fetchAMPStatus(String key, int tries) throws IOException {
        ...
        if (responseCode == 405){
            if(tries < 3){
                ...
                return new AMPIntegration.AMPStatus(decodeB64("NDA1LCBTb21ldGhpbmcgd2VudCB3cm9uZw=="), false);
            }
        }
        ...
        if (responseCode == 400){
            if(tries < 3){
                ...
                return new AMPIntegration.AMPStatus(decodeB64("NDAwLCBTb21ldGhpbmcgd2VudCB3cm9uZy4="), false);
            }
        }
        ...
        if (responseCode == 500){
            if(tries < 3){
                ...
                return new AMPIntegration.AMPStatus(decodeB64("NTAwLCBTb21ldGhpbmcgd2VudCB3cm9uZw=="), false);
            }
        }
        ...
        AMPIntegration.AMPStatus licenseStatus = new AMPIntegration.AMPStatus(value, value.equals(decodeB64("RU5BQkxFRA=="))); // "ENABLED"

        if (responseCode == 404){
            if(tries < 3){
                return fetchAMPStatus(key, tries + 1);
            } else {
                return new AMPIntegration.AMPStatus(decodeB64("SW52YWxpZCBMaWNlbnNlOiA=") + value, false);
            }
        } else {
    ...
    public AMPIntegration.AMPStatus fetchStatus(String key) {
        try {
            return fetchAMPStatus(key, 0);
        } catch (IOException e) {
            e.printStackTrace();
            return new AMPIntegration.AMPStatus(decodeB64("VW5hYmxlIHRvIGNvbm5lY3Qu"), false);
        }
    }
