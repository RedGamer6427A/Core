    ...
    public GlobalChatCommand() {
        ...
        addSyntax(context -> {
            ...
            future.thenAccept(integer -> {
                if (integer != 0) {
                    answer(context, "<red>An exception occurred while sending this message!", "<red>An exception occurred while sending this message: <dark_red>"+ MessageBusBrokerResponse.fromCode(integer));
                }
            });
