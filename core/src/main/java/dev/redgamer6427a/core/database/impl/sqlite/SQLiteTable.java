package dev.redgamer6427a.core.database.impl.sqlite;

import dev.redgamer6427a.core.database.core.DBUtil;
import dev.redgamer6427a.core.database.core.Table;

import java.util.List;

/**
 * A High-Level No-SQL MariaDB Table implementation.
 *
 * @param <K> The Key/Primary Type.
 * @param <R> The Record to indicate the database fields.
 */
public abstract class SQLiteTable<K, R extends Record> extends Table<K, R> {


    /**
     *
     * @param tableName   The name of the table.
     * @param database    The parent database.
     * @param keyClass    The class of the Key
     * @param recordClass The class of the Record.
     */
    protected SQLiteTable(String tableName, SQLiteDatabase database, Class<K> keyClass, Class<R> recordClass) {
        super(tableName, database, keyClass, recordClass, DBUtil.Dialect.SQLITE);
    }

    @Override
    protected String buildUpsertSuffix(List<TableColumn> columns) {
        return ")";
    }

}
